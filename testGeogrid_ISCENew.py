...
def cmdLineParse():
    ...
    parser.add_argument('-b', '--buffer', dest='buffer', type=int, required=False, default=0,
    ...
    parser.add_argument('--orbit_dir', help='Directory of Sentinel-1 orbit files')
    parser.add_argument('--aux_dir', help='Directory of Sentinel-1 aux files')
...
def loadMetadata(indir_m,buffer=0):
    '''
    Input file.
    '''
    import os
    ...
    for swath in range(1,4):
        inxml = os.path.join(indir_m, 'IW{0}.xml'.format(swath))
    ...
    #pdb.set_trace()
    print("Jing Print startingRange")
    print(info.startingRange)
    print("Jing Print farRange")
    print(info.farRange)
...
def loadParsedata(indir_m, orbit_dir, aux_dir, buffer=0):
    '''
    Input file.
    '''
    import os
...
#        rdr.safe=['./S1A_IW_SLC__1SDH_20180401T100057_20180401T100124_021272_024972_8CAF.zip']
        ...
        rdr.polarization=get_polarizations(indir_m)[0]
        rdr.parse()
...
def main():
    ...
    if inps.optical_flag == 1:
        ...
        if inps.parse:
            metadata_m = loadParsedata(inps.indir_m, inps.orbit_dir, inps.aux_dir, inps.buffer)
            print("main buffer")
            print(inps.buffer)
            metadata_s = loadParsedata(inps.indir_s, inps.orbit_dir, inps.aux_dir, inps.buffer)
            print("main buffer")
            print(inps.buffer)
        else:
